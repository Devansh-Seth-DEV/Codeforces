	...
	public enable_shared_from_this<SolutionDelegate>
{
	...
	public:
		...
		template <typename T>
			void didOut(const vector<vector<T>>& out,
					const char *sep = " ",
					const char *end = "\n") {
				for(const auto& o: out) {
					for(const auto& v: o) cout << v << sep;
					cout << end;
				}
			}
...
class ProblemSolver: public SolutionDelegate {
	public:
		void execute() override {
			...
			auto res = solve(x, y, k); //Solver function (modify parameters accordingly)
			didOut(res); // Display's the result on console
		}

	private:
		vector<vector<int>> solve(const int x, const int y, const int k) {
