	...
	public:
		void execute() override {
			...
			for(int i=0; i<m; i++) {
			    ...
			    int l, r;
			    cin >> c >> l >> r;
			    opr.push_back({(int) c, l, r});
			}
			
			auto res = solve(us, opr); //Solver function (modify parameters accordingly)
			didOut(res); // Display's the result on console
		}

	private:
		vector<long long> solve(const unordered_set<int>& set, const vector<vector<int>>& opr) {
